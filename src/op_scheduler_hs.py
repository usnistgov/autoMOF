...
def reactor_use_assignment(reactor_df, number_of_reactors, positions_in_reactor):
  """
  This function reahs the reactor dataframe
  ...
  reactor_df = reactor_df.sort_values("Total Reactor Duration (hs)", ascending = False)
  ...
  if np.max(reactors_required) >= number_of_reactors:
    second_to_last_duration = reactor_df["Total Reactor Duration (hs)"].to_numpy()[-2]
    second_to_last_index = reactor_df[reactor_df["Total Reactor Duration (hs)"] == second_to_last_duration].index[0]


    last_duration = reactor_df["Total Reactor Duration (hs)"].to_numpy()[-1]
    last_index = reactor_df[reactor_df["Total Reactor Duration (hs)"] == last_duration].index[0]

    #Pair the last 2 rows together:
    use1 = reactor_df.iloc[-1]["Reactor Use (hs)"]
    temp1 = reactor_df.iloc[-1]["Reactor Temperature (C)"]
    use2 = reactor_df.iloc[-2]["Reactor Use (hs)"]
    ...
    reactor_df.at[second_to_last_index, "Reactor Use (hs)"] = total_use
    reactor_df.at[second_to_last_index, "Reactor Temperature (C)"] = total_temp
    reactor_df.at[second_to_last_index, "Total Reactor Duration (hs)"] = total_duration
...
def assign_reactors(unit_ops_df, number_of_reactors, positions_in_reactor):
  ...
  for temp in unique_temperatures:
    ...
    total_react_duration.append(np.max(unit_ops_df[unit_ops_df["UnitOP"]=='react'][temp_mask]["Duration (hs)"]))
  ...
  reactor_df["Total Reactor Duration (hs)"] = total_react_duration
  reactor_df["Reactor Use (hs)"] = list_of_react_durations #Needs to be a list
  ...
  reactor_df['Reactor Use (hs)'] = reactor_df['Reactor Use (hs)'].astype("object")
...
def create_unit_ops_df(sample_db, 
    ...
    Durations are in hs - so that there is fine enough fidelity, but still an integer (as needed for constraint solver)
    ...
    header = ["Sample Name", "UnitOP", "Duration (hs)", "Reactor", "Reactor Temperature (C)"]
    unit_ops_df = pd.DataFrame(columns = header)
    for key in sample_db.keys():
        
        if Add_fluids == True:
            sub_df = pd.DataFrame([[key, "add_fluids", 2*6/10, None, 0]], columns = header)
        ...
        if React == True:
            ...
            sub_df = pd.DataFrame([[key, "react", time*6/10, None, temp]], columns = header)
            unit_ops_df = pd.concat([unit_ops_df, sub_df])

        for i in range(Wash_Cycles):
            sub_df = pd.DataFrame([[key, f"centrifuge_{i}", 40*6/10, None, 0],
                                   [key, f"rm_supernatent_{i}", 2*6/10, None, 0],
                                   [key, f"add_solvents_{i}", 2*6/10, None, 0],
                                   [key, f"sonicate_{i}", 60*6/10, None, 0],
                                   [key, f"rack_hold_{i}", 24*60*6/10, None, 0]], 
                                   columns = header)
            unit_ops_df = pd.concat([unit_ops_df, sub_df])

        if Dry == True:
            sub_df = pd.DataFrame([[key, f"centrifuge_{0+Wash_Cycles}", 40*6/10, None, 0],
                                   [key, f"rm_supernatent_{0+Wash_Cycles}", 2*6/10, None, 0],
                                   [key, "dry", 24*60*6/10, None, 100]], 
                                   columns = header)
            unit_ops_df = pd.concat([unit_ops_df, sub_df])
           

        if Centrifuge == True:
            sub_df = pd.DataFrame([[key, "centrifuge", 40*6/10, None, 0]], columns = header)
            unit_ops_df = pd.concat([unit_ops_df, sub_df])

        if Remove_supernatent == True:
            sub_df = pd.DataFrame([[key, "rm_supernatent", 2*6/10, None, 0]], columns = header)
            unit_ops_df = pd.concat([unit_ops_df, sub_df])
        
        if Sonicate == True:
            sub_df = pd.DataFrame([[key, "sonicate", 60*6/10, None, 0]], columns = header)
            unit_ops_df = pd.concat([unit_ops_df, sub_df])
...
def solve_job_shop_schedule(unit_ops_df, num_reactors, plot = False):
    ...
    horizon = int(np.sum(np.ceil(unit_ops_df["Duration (hs)"])))
    ...
    for i, sample in enumerate(samples):
        ...
        for j in sub_df.index:
            task = Task(task_id=j,
                        ...
                        duration= sub_df.loc[j, "Duration (hs)"],
    ...
    # Add_fluids.end should be within 4 hs of react.start for each sample
    for job in batch.jobs:
        add_fluids = None
        react = None
        for task in job.tasks:
        ...
        if (add_fluids is not None) and (react is not None):
            batch.model.add(add_fluids + 4 > react)
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        unit_ops_df["Start Time (hs)"] = None
        unit_ops_df["End Time (hs)"] = None
        ...
        for job in batch.jobs:
            ...
            for task in job.tasks:
                ...
                if "dry" not in task_name:
                    unit_ops_df.loc[unit_op_table_index, "Start Time (hs)"] = solver.value(task.start_var)
                    unit_ops_df.loc[unit_op_table_index, "End Time (hs)"] = solver.value(task.end_var)
                    ...
                    unit_ops_df.loc[unit_op_table_index, "Start Time (hs)"] = solver.value(task.start_var[index])
                    unit_ops_df.loc[unit_op_table_index, "End Time (hs)"] = solver.value(task.end_var[index])
        ...
        unit_ops_df = unit_ops_df.sort_values("Start Time (hs)")
...
def interleave_reactor_preheating(unit_ops_df, heating_rate):
  ...
  heating_rate_hs = heating_rate * 100 /60 #Convert to C/hs

  # Sort the DataFrame by 'Reactor', 'Start Time (hs)'
  df_react = unit_ops_df[unit_ops_df['Reactor'].notnull()]
  df_react = df_react.sort_values(by=['Reactor', 'Start Time (hs)'])
      ...
      for i, (idx, row) in enumerate(reactor_df.iterrows()):
          if row['UnitOP'] == 'react':
              ...
              if row['Reactor Temperature (C)'] != prev_temp:# and prev_temp is not None:
                  ...
                  duration = (current_temp - prev_temp)/heating_rate_hs # Time to get to temperature in hs
                  
                  new_row['Duration (hs)'] = duration
                  if i == 0:
                      new_row['Start Time (hs)'] = row['Start Time (hs)'] - duration  # Adjust start time
                      new_row['End Time (hs)'] = new_row['Start Time (hs)'] + duration  # End time is the same as start time
                  else:
                      prev_end_time = reactor_df.iloc[i -1]['End Time (hs)']
                      new_row['Start Time (hs)'] = prev_end_time 
                      new_row['End Time (hs)'] = prev_end_time + duration
                  new_row['Op Order'] = 0 
  ...
  unit_ops_df = pd.concat([unit_ops_df, new_rows_df]).sort_values(by=['Start Time (hs)', 'Op Order']).reset_index(drop=True)
  ...
  for idx, pre_heat_row in pre_heat_rows.iterrows():
      ...
      if not react_row.empty:
          ...
          shift_amount = pre_heat_row['End Time (hs)'] - react_row['Start Time (hs)']
  ...
  for react_idx, shift_amount in shift_amounts.items():
      ...
      subsequent_react_rows = unit_ops_df[(unit_ops_df['UnitOP'] == 'react') & 
                                ...
                                (unit_ops_df['End Time (hs)'] == unit_ops_df.loc[react_idx, 'End Time (hs)'])]
      
      # Adjust the 'react' step
      unit_ops_df.loc[react_idx, 'Start Time (hs)'] += shift_amount
      unit_ops_df.loc[react_idx, 'End Time (hs)'] += shift_amount
      ...
      unit_ops_df.loc[subsequent_rows.index, 'Start Time (hs)'] += shift_amount
      unit_ops_df.loc[subsequent_rows.index, 'End Time (hs)'] += shift_amount
      ...
      for subsequent_react_idx in subsequent_react_rows.index:
          if subsequent_react_idx != react_idx:
              ...
              unit_ops_df.loc[subsequent_react_idx, 'Start Time (hs)'] += shift_amount
              unit_ops_df.loc[subsequent_react_idx, 'End Time (hs)'] += shift_amount
              ...
              unit_ops_df.loc[other_subsequent_rows.index, 'Start Time (hs)'] += shift_amount
              unit_ops_df.loc[other_subsequent_rows.index, 'End Time (hs)'] += shift_amount
  
  #Shift all the ops so that the first op starts at time 0
  min_start = unit_ops_df["Start Time (hs)"].min()

  if min_start < 0:
      shift = np.abs(min_start)

      unit_ops_df["Start Time (hs)"] += shift
      unit_ops_df["End Time (hs)"] += shift
