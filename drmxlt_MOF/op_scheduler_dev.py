...
def find_op_order(unit_ops_df):
    """
    Function to extract the list of UnitOPs from the unit_ops_df.
    Assumes each sample has the same steps.
    ...
    Returns
    op_order : list
        List of strings of the name of each unit op
    """
    op_order = unit_ops_df[unit_ops_df["Sample Name"] == unit_ops_df["Sample Name"][0]]["UnitOP"].to_list()
    return op_order

def build_job_shop_var_containers(unit_ops_df, num_reactors):
    """
    ...
    #Get the list of each UnitOP
    op_order = find_op_order(unit_ops_df)
...
def solve_job_shop_schedule(unit_ops_df, num_reactors, plot = False):
    ...
    resource_dict, interval_dict = build_job_shop_var_containers(unit_ops_df, num_reactors)
    ...
    model.AddCumulative(sonicator_intervals, demands, 1)
