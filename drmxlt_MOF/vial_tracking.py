...
def loaded_vial_racks(vial_racks = "left", empty_rack_left_indexes = [], empty_rack_right_indexes = []):
    ...
    if vial_racks == "right":
      ...
      return loaded_rack_right

    if vial_racks == "both":
      loaded_rack_left = loaded_vial_racks("left", empty_rack_left_indexes)
      loaded_rack_right = loaded_vial_racks("right", empty_rack_right_indexes)
...
left_rack_assignments = np.where(left_rack_assignments == 1, "Unassigned", "0")

right_rack_assignments = np.ones_like(vial_rack_right_array) * loaded_rack_right
right_rack_assignments = np.where(right_rack_assignments == 1, "Unassigned", "0")
